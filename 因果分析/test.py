...
class randomval:
    ...
    def limit_number(self, number, min_range, max_range):
        if number < min_range:
            number = min_range
    ...
    def doe_expand(self, datainput, mode='doe', index=0):
        ...
        for i in range(len(datainput)):
            ...
            for j in range(-int(valuelen/2), int(valuelen/2)):
                datatmp = self.limit_number(datainput[i] + j / self.difflen[i], 0.0,1.0)
                dic[i].append(datatmp)
    ...
    def predict(self, batch_size, datainput, expectedoutput, errorratio, mode='doe', device=torch.device("cpu"), count=1):
        ...
        model = GruRNN(INPUT_FEATURES_NUM, 128, output_size=OUTPUT_FEATURES_NUM, num_layers=1).to(device)
...
def auto_testmode(data_y):
    ...
    with open("scalerx.pkl", "rb") as infile:
        ...
        with open("scalery.pkl", "rb") as infile:
            ...
            return np.asarray(datatmp2).reshape(1,-1)

def getCurTrainEpoch():
    global curepoch
    return curepoch
def train_mode(data, dataxdim):
    device = torch.device("cpu")
    ...
    data_x, data_y, data_col, pear, diffvalue, difflen, dataprex, dataprey, data_y_field = DataRead().data_read(data, dataxdim)
