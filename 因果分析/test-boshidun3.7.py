...
class DataPreprocess:
    ...
    def pearson(self, data, outputdim, savename):
        ...
        cols = [x for i, x in enumerate(restmp.columns) if np.fabs(restmp.iat[0, i]) > 0.6]
        logging.info(cols)
...
class GruTrain(object):
    ...
    def train_gru_model(self, data_y_field, device=torch.device("cpu")):
        ...
        #     plt.savefig(data_y_field[i] + ".svg")
...
class randomval:
    ...
    def predict(self, batch_size, datainput, expectedoutput, errorratio, mode='doe', device=torch.device("cpu"), count=1):
        ...
        if mode != 'doe':
            ...
            if flag == 0:
                ...
                return self.predict(batch_size, data_x[losslist[0]], expectedoutput, errorratio, mode, device, count)
...
def gru_testmodel(data_x, OUTPUT_FEATURES_NUM):
    ...
    with open("scalerx.pkl", "rb") as infile:
        ...
        data_x = scalerx.transform(data_x)
...
if __name__ == '__main__':
    ...
    # data_x, data_y, data_col, pear, diffvalue = DataRead().data_readtmp("E:\BaiduNetdiskDownload//real\CSV\CSV\data_line.csv","E:\BaiduNetdiskDownload//real\CSV\CSV//new.csv")
